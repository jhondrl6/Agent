...
import { useGlobalAgentStatus, AgentStatusData } from '@/hooks/useGlobalAgentStatus';
...
export function ProgressMonitor() {
  ...
  const { statusData: globalStatusData, isLoading: isGlobalStatusLoading, error: globalStatusError } = useGlobalAgentStatus();
  ...
  return (
    <div className="p-6 my-4 bg-white border border-gray-200 rounded-xl shadow-lg">
      ...
      {/* Global Backend Agent Status Section */}
      <div className="mt-3 pt-3 border-t border-gray-200">
        <h4 className="font-semibold text-gray-700 mb-1">Backend Engine Status:</h4>
        {isGlobalStatusLoading && !globalStatusData ? (
          ...
          <p className="text-red-600 bg-red-50 p-2 rounded-md text-sm">
            Error fetching backend status: {globalStatusError.message}
          </p>
        ) : globalStatusData ? (
          globalStatusData.isActive ? (
            <p className="text-blue-600">
              Backend Engine: Active (processing {globalStatusData.activeMissionsCount} mission(s))
            </p>
          ) : (
            <p className="text-green-600">Backend Engine: Idle</p>
          )
        ) : (
          <p className="text-gray-500 italic">Backend status unavailable.</p> // Should ideally not be reached if isLoading/error are handled
        )}
         {isGlobalStatusLoading && globalStatusData && ( // Show subtle loading indicator during polling refreshes
            <p className="text-xs text-gray-400 mt-1">(Refreshing backend status...)</p>
        )}
      </div>
    </div>
  );
